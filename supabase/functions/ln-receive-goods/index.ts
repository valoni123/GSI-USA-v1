...
serve(async (req) => {
  try {
    ...
    const requiredFields = ["OrderOrigin", "Order", "Position", "Sequence", "Set", "PackingSlip", "Quantity", "Unit", "Confirm", "FromWebservice"];
    const missing = requiredFields.filter((k) => (body as any)[k] === undefined || (body as any)[k] === null || ((typeof (body as any)[k] === "string") && !(body as any)[k].trim()));
    if (missing.length) {
    ...
    const postRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        Set: body.Set,
        PackingSlip: body.PackingSlip,
        Lot: body.Lot ?? "",
        BusinessPartnerLot: body.BusinessPartnerLot ?? "",
        Quantity: body.Quantity,
        Unit: body.Unit,
        Confirm: body.Confirm,
        FromWebservice: body.FromWebservice,
      }),
    }).catch(() => null as unknown as Response);
