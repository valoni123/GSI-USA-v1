...
serve(async (req) => {
  try {
    ...
    // 1) Transport order lookup (exact-first filter to reduce OData cost, fallback with endswith)
    ...
    let odataRes = await fetchOData(cfg, company, exactUrl, language);
    ...
    // Determine chosen HU for subsequent info checks
    const chosenHU = ((orders.first?.HandlingUnit || "") as string).trim();

    // 2) Loaded check (only if HU present and vehicleId provided)
    let loadedCount = 0;
    ...
    if (chosenHU && vehicleId) {
      ...
      const lcRes = await fetchOData(cfg, company, lcUrl, language);
      const lcJson: any = await lcRes.json().catch(() => null);
      if (lcRes.ok && lcJson) {
        loadedCount = Number(lcJson["@odata.count"] ?? 0);
      } else {
        // If it errors, keep loadedCount as 0 and continue
        loadedCount = 0;
      }
    }
