...
serve(async (req) => {
  try {
    ...
    // OData call for ItemWarehousing by key
    const base = iu.endsWith("/") ? iu.slice(0, -1) : iu;
    const escaped = item.replace(/'/g, "''"); // preserve spaces as provided
    const path = `/${ti}/LN/lnapi/odata/whapi.wmdItemWarehousing/ItemWarehousing(Item='${escaped}')`;
    const url = `${base}${path}?$select=LotTracking&$expand=ItemRef`;
    ...
    // For key path, response should be an object, not an array
    const entity = (odataJson && typeof odataJson === "object" && !Array.isArray(odataJson)) ? odataJson : null;
    if (!entity) {
      return json({ ok: false, error: { message: "not_found" } }, 200);
    }
    const raw = (entity?.LotTracking ?? "").toString().toLowerCase();
    const lotTracking = raw === "yes" || raw === "true";

    return json({ ok: true, lotTracking }, 200);
  } catch {
