...
const addCoverPage = (doc: jsPDF, logoDataUrl: string | null, title: string) => {
  ...
  if (logoDataUrl) {
    const logoWidth = 220; // px
    const logoHeight = 80; // px approximate
    const x = (pageWidth - logoWidth) / 2;
    const y = 80;
    doc.addImage(logoDataUrl, "PNG", x, y, logoWidth, logoHeight);
  }

  // Main label (Benutzerhandbuch or localized)
  const coverLabel = getCoverLabel(title);
  doc.setTextColor(0, 0, 0);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(26);
  doc.text(coverLabel, marginX, 200);

  // Subtitle (original title)
  doc.setFont("helvetica", "normal");
  doc.setFontSize(14);
  doc.text(title, marginX, 230);

  // Footer note placeholder
  doc.setFont("helvetica", "italic");
  doc.setFontSize(10);
  const footer = "Generated by the application. For internal use.";
  doc.text(footer, marginX, pageHeight - 50);
};
